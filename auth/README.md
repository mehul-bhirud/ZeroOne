...
Set `DATABASE_URL`, `JWT_SECRET`, `JWT_ISSUER`, `JWT_AUDIENCE`, `JWT_TTL_SECONDS`, and `AUTH_PORT` before running `npm run dev:auth`. Passwords use Node scrypt; JWTs contain only the User subject and are checked against the current database User on every authenticated request.
